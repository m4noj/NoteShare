...
if(!$user = $db->selectOne('users', ['id' => $user_id])){

    $_SESSION['flash_messages'][]=['type'=>'danger','message'=>'user not found!'];
...
<div class="container mt-5 pt-4">
  <div class="row">
    ...
    <div class="col-md-4">
      ...
      <div class="user-sidebar">
              ...
              <h2><?php echo $user['first'].' '.$user['last'];?><?php if (isset($_SESSION['user']['id']) === $user_id): ?><span class="text-muted"> (me)</span><?php endif;?></h2>
              ...
              <?php if (isset($_SESSION['user']['id']) === $user_id): ?>
                <a href="profile_edit.php" class="btn btn-primary btn-sm">Edit Profile</a>

                  <?php elseif(($user['role']==='student') && ($_SESSION['user']['id']!=$user_id)) : ?>

                    <form action="friend_request.php" method="POST">
                  <input type="hidden" name="receiver_id" value="<?= $user['id'] ?>">
                  <button type="submit">Add Friend</button>
                  </form>
                  <!-- <a href="../app/friend_request.php?id=<?=$user_id?>" class="btn btn-primary btn-sm">Add Friend</a> -->
                
                  <?php elseif(($user['role']==='expert') && ($_SESSION['user']['id']!=$user_id)): ?>

                        <form action="../app/follow_request.php" method="POST">
  ...
  showSection('posts',document.getElementById('btnpost'));
      ...
      <div class="col-md-8">
        ...
        <button class="btn btn-outline-primary active" onclick="showSection('posts', this)" id="btnpost"><?php if (isset($_SESSION['user']['id']) === $user_id): ?>My<?php endif;?> Posts</button>
        <button class="btn btn-outline-primary" onclick="showSection('comments', this)"><?php if (isset($_SESSION['user']['id']) === $user_id): ?>My<?php endif;?> Comments</button>
        <button class="btn btn-outline-primary" onclick="showSection('communities', this)"><?php if (isset($_SESSION['user']['id']) === $user_id): ?>My<?php endif;?> Communities</button>
          <button class="btn btn-outline-primary" onclick="showSection('friends', this)"><?php if (isset($_SESSION['user']['id']) === $user_id): ?>My<?php endif;?> Friends</button>
            <button class="btn btn-outline-primary" onclick="showSection('stats', this)"><?php if (isset($_SESSION['user']['id']) === $user_id): ?>My<?php endif;?> Reputation</button>
